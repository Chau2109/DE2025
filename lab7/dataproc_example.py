...
df = spark.read \
  .format("bigquery") \
  .load("dechau.labdataset.retaildata")    # project_id.datatset.tablename. Use your project id
df.printSchema()
df.show(4)
...
bucket = "chautemp_de2025"  # use your bucket 
...
ordered_word_count.write.format('bigquery') \
  .option('table', 'dechau.labdataset.wordcounts') \
  .mode("append") \
  .save()
